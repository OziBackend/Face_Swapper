...
def face_swap_func(image_copy, swapper, model, filename):
    ...
    print(FUNCTION_FACE_SWAP)   #DEBUG
    
    #Defining Output File Path
    unique_id = str(uuid.uuid4())
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S") 
    output_file_path = os.path.join(STATIC_PATH, f"{filename.split('.')[0]}_{unique_id}_{timestamp}.webp")
    ...
    try:
        ...
        print(FACES_DETECTED, len(faces))   #DEBUG

        if not faces:
            print(NO_FACES_DETECTED)   #DEBUG
            raise ValueError(NO_FACES_DETECTED)

        if len(faces) < 2:
            print(LESS_FACES_DETECTED)   #DEBUG
            raise ValueError(LESS_FACES_DETECTED)
        elif len(faces) > 2:
            print(MORE_FACES_DETECTED)   #DEBUG
            raise ValueError(MORE_FACES_DETECTED)
        ...
        if swapped_image is None or swapped_image.size == 0:
            raise ValueError(FAILED_TO_SWAP_FACES)
        
        # Save swapped image using cv2
        success = cv2.imwrite(output_file_path, swapped_image)
        if not success:
            raise ValueError(f"{FAILED_TO_SAVE_IMAGE} {output_file_path}")
        
        print(f"{IMAGE_SAVED_SUCCESSFULLY} {output_file_path}")   #DEBUG
        image_url = f"{IMAGE_URL_PREFIX}{output_file_path}"
        return {
            "message": SUCCESS_SWAPPED_IMAGE,
            "image_url": image_url
        }

    except Exception as e:
        return {
            "message": f"{ERROR_SWAPPING_FACES} {e}",
            "image_url": None
        }
